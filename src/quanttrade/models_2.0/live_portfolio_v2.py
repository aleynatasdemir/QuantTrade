...
- Sonraki çalıştırmalarda:
    ...
    - Mevcut pozisyonlara STOP / MODEL+TIME EXIT uygular (STOP aynı gün, diğerleri T+1 planlı)
...
TOP_K           = 5         # günlük en iyi K hisse (model skoru)
...
def load_state():
    ...
    if os.path.exists(STATE_PATH):
        ...
        state = {
            ...
            "positions": [],      # [{symbol, entry_price, shares, entry_date(str), days_held, exit_planned?, exit_reason_planned?}]
...
def calculate_stagnation_indicators(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Backtest kodundakiyle aynı mantık:
      - natr < 2.5 ve trend_dev < 1.5%  -> is_stagnant
      - stagnant_3d_count >= 3          -> son 3 gün ölü hisse
      - pct_change_5d < -2%             -> is_rs_weak
    """
    ...
    # True Range
    df["tr0"] = (df[HIGH_COL] - df[LOW_COL]).abs()
    df["tr1"] = (df[HIGH_COL] - df[PRICE_COL].shift(1)).abs()
    df["tr2"] = (df[LOW_COL] - df[PRICE_COL].shift(1)).abs()
    ...
    # Trend sapması (20 günlük ortalama etrafı)
...
def main():
    ...
    # Eski state'teki pozisyonlara default flag'ler
    for p in positions:
        if "exit_planned" not in p:
            p["exit_planned"] = False
        if "exit_reason_planned" not in p:
            p["exit_reason_planned"] = None
    ...
    for current_date in new_dates:
        ...
        # Bugünün TOP_K listesi (model skoru)
        today_sorted = today_data.sort_values("score", ascending=False)
        top_symbols = set(today_sorted.head(TOP_K).index)

        # --- 1.a) Önceden planlanan EXIT'leri bugünün açılışında gerçekleştir (T+1 satış) ---
        if positions:
            for i in range(len(positions) - 1, -1, -1):
                pos = positions[i]
                if not pos.get("exit_planned", False):
                    continue

                sym = pos["symbol"]
                if sym not in today_data.index:
                    # Hisse bugün işlem görmüyorsa planı bozma, ertesi güne dene
                    pos["days_held"] += 1
                    continue

                open_price = float(today_data.loc[sym, OPEN_COL])
                exit_price = open_price  # burada ekstra slipaj yok, istersen ekleyebilirsin
        ...
        # --- 1.b) Pending BUY emirlerini bugünün açılışında gerçekleştir ---
        if pending_buys:
            new_pending = []
        ...
        # --- 1.c) MEVCUT POZİSYONLARA STOP + MODEL/TIME EXIT PLANLAMA ---
        for i in range(len(positions) - 1, -1, -1):
            pos = positions[i]
            sym = pos["symbol"]

            # Zaten exit planlı ise, bugünkü OHLC'ye göre tekrar karar verme
            ...
            # A) Performance Fail (RELAXED)
            if (days >= 8) and (ret_close < -0.02) and (sym not in top_symbols):
                exit_planned = True
                exit_reason_planned = "PERF_FAIL_RELAXED"

            # B) Stagnation Exit
            stagnant_days = row.get("stagnant_3d_count", 0) or 0
            if (not exit_planned) and (days > 10) and (stagnant_days >= 3) and (ret_close < 0.03):
                exit_planned = True
                exit_reason_planned = "STAGNATION_EXIT_RELAXED"

            # C) Relative Weakness Exit
            if (not exit_planned) and (days > 5) and row.get("is_rs_weak", False) and (sym not in top_symbols):
                exit_planned = True
                exit_reason_planned = "WEAK_RS_EXIT"

            # D) Time Exit (HORIZON)
            if (not exit_planned) and (days >= HORIZON_DAYS):
                exit_planned = True
            ...
            if exit_planned:
                ...
                pos["exit_reason_planned"] = exit_reason_planned

            # Gün sayısını artır
            pos["days_held"] = days + 1
    ...
    if positions:
        ...
        for p in positions:
            ...
            last_px = float(p.get("current_price", entry))
            pnl_pct = (last_px / entry - 1.0) * 100.0
            pnl_tl = (last_px - entry) * shares

            status = ""
            if p.get("exit_planned", False):
                status = f"  [YARIN SAT ({p.get('exit_reason_planned')})]"

            print(
                f"  {sym}: {shares} adet, giriş {entry:.2f}, "
                f"son {last_px:.2f}, PnL {pnl_pct:.2f}% ({pnl_tl:,.0f} TL), "
                f"gün sayısı {days_held}{status}"
            )
    else:
        print("\nAktif pozisyon yok.")
